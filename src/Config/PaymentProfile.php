...
class PaymentProfile
{
    ...
    public static function fromConfig(array $config): PaymentProfile
    {
        $paymentProfile = new PaymentProfile();
        $paymentProfile->setType((string) $config['type']);
        $paymentProfile->setAuthRequired((bool) $config['auth_required']);
        $paymentProfile->setReturnUrlOverride($config['return_url_override'] ?? null);
        $paymentProfile->setReturnUrlExpression((string) $config['return_url_expression']);
        $paymentProfile->setNotifyUrlExpression((string) $config['notify_url_expression']);
        $paymentProfile->setPspReturnUrlExpression((string) $config['psp_return_url_expression']);
        $paymentProfile->setDataProtectionDeclarationUrl($config['data_protection_declaration_url'] ?? null);
        $paymentProfile->setRecipient($config['recipient'] ?? null);

        $concurrencyLimits = $config['concurrency_limits'];
        $paymentProfile->setMaxConcurrentPayments($concurrencyLimits['max_concurrent_payments'] ?? null);
        $paymentProfile->setMaxConcurrentAuthPayments($concurrencyLimits['max_concurrent_auth_payments'] ?? null);
        $paymentProfile->setMaxConcurrentAuthPaymentsPerUser($concurrencyLimits['max_concurrent_auth_payments_per_user'] ?? null);
        $paymentProfile->setMaxConcurrentUnauthPayments($concurrencyLimits['max_concurrent_unauth_payments']);
        $paymentProfile->setMaxConcurrentUnauthPaymentsPerIp($concurrencyLimits['max_concurrent_unauth_payments_per_ip'] ?? null);

        $notifyErrorConfig = $config['notify_error'] ?? null;
        if ($notifyErrorConfig !== null) {
            $paymentProfile->setNotifyErrorConfig(new NotifyErrorConfig($notifyErrorConfig));
        }

        $reportingConfig = $config['reporting'] ?? null;
        if ($reportingConfig !== null) {
            $paymentProfile->setReportingConfig(new ReportingConfig($reportingConfig));
        }
